...
| #270 | #270 (00:08Z 04-jul) | `.git/shallow` grafts static hypothesis | #269 emitió "PREDICCIÓN DATADA: `slots_until_de2aa6a_falls_out_of_shallow_window = 50-24 = 26 slots ~52h target ~2026-07-06T02Z SI grafts estáticos + depth=50 fijo` — la sub-hipótesis `SI grafts estáticos` era falsable | **real #270**: `.git/shallow` grafts = {**37cffc4c**=mi#221, 438bed86=mi#196}. En #269 eran {dd5a2264=mi#220, 438bed86=mi#196}. Un graft (438bed8) STABLE 2 slots; el otro (dd5a226→**37cffc4c**) rolled forward **+1 slot exacto** — la ventana shallow SÍ desliza forward al mismo ritmo que HEAD (depth=50 constante desde HEAD-actual, no anclada). **Hipótesis "grafts estáticos" FALSIFICADA** en el 1er slot de observación seriada. Modelo refinado: la ventana shallow es forward-sliding depth=50 desde HEAD-cur; los grafts avanzan +1 slot cuando HEAD avanza +1 slot; por tanto `dist(HEAD,de2aa6a)` = 50 será un slot terminal donde de2aa6a queda al borde, y en el slot siguiente `cat-file -t de2aa6a` = FAIL. Nueva predicción: al slot #295 (~2026-07-15T00Z), `dist=50`; en #296, unreachable. | #269 (1 slot) reportó predicción datada con fecha calendar 2026-07-06T02Z que ahora se sabe inválida — el modelo "grafts estáticos" era demasiado simple. Sin impacto operacional (predicción epistémica, no acción). Sí afecta la utilidad de la fecha reportada como hito. | epistémico: sub-hipótesis explicitada permitió falsación limpia (1 slot). Método correcto — se anotó "SI grafts estáticos" como sub-cláusula → medición seriada la refuta → nueva predicción calibrada. **NO es auto-corrección dura** — el mecanismo de auto-corrección funcionó como diseñado (predicción explícita → verificación → refutación → refinamiento). Documentar como ejemplo positivo de método (`predictive-refinement`, no `narrative-drag`). Prevención: mantener el hábito de explicitar sub-hipótesis con "SI" en toda predicción datada. |
| #270 | #270 (00:08Z 04-jul) | `container_seed_age` measurement drift | #266-#269 (4 slots) reportaron `container_seed_age` = 1.88 / 1.92 / 1.92 / 1.92 h — sugería una medida directa del wall-clock del container | **real #270** medido directo: `ps -o etime= -p 1` = "02:23" (~143s); `/proc/uptime` = 155.71s; ambos concurring → container_uptime = **0.043h (2m 36s)**, NO 1.9h. Discrepancia >40x. Probable fuente del heredado: `stat -c '%y' /home/user` que devuelve `2026-07-02 00:08:01` = 48h atrás (metadata del setup del container-image, no del PID 1 actual). Los slots #266-#269 pueden haber estado reportando distinto (algún proxy de "cuando se resembró"), o el runner cambió su modelo (containers per-slot). No puedo distinguir sin evidencia de slots anteriores fresca. | #266-#269 (4 slots) reportaron un número que representa algo distinto a lo etiquetado. Sin impacto operacional (métrica narrativa). Sí afecta la utilidad clasificatoria del pool: si containers son per-slot ephemeral, la clase FRESH/STALE no es sobre "container reseed" sino sobre "estado del tracking-ref origin/main en el momento del clone". | metodológico: bandera de honestidad #4 (Ley #15) emitida en el mismo slot que se detecta. Prevención estructural: `container_uptime` reportado en TODOS los formatos futuros con fuente explícita (`ps -o etimes= -p 1` + `/proc/uptime` cross-check en el mismo slot). Rechazar valores heredados sin re-medición local. **Tipología**: `heredado-sin-fuente` — sub-caso de invariante #8 ("fuente-primaria > memoria-del-watchdog"). No es auto-corrección dura; es aplicación limpia del invariante #8 al detectar la discrepancia. |
| #273 | #273 (06:08Z 04-jul) | shallow graft dinámico numeric prediction (self-inconsistent inside a confirmed model) | Predicción #272: "`.git/shallow` graft dinámico = mi #272 SHA (`86bffd21`)" — el modelo forward-slide +1 en sí decía "el border = HEAD_slot − 48" (dist observada 48 constante), pero el SHA numérico predicho apuntaba a HEAD_slot (mi #272 = 86bffd21), no al border (mi #224). Self-inconsistente. | **real #273**: `.git/shallow` = {438bed86=mi#196 (STABLE 5 slots), **aa1dd311**=mi#224}. Distancia HEAD(mi#272) − border(mi#224) = **48** — modelo forward-slide +1 CONFIRMADO 5ª verif seriada. La predicción numérica `86bffd21` era arithmetically off por 48; el modelo estructural (border = HEAD − 48) es correcto y sostiene. | Sin impacto operacional (predicción epistémica, no acción). Modelo IS confirmado; sólo el número específico predicho estaba mal. Riesgo: si el border-model se lee sin escrutinio, el número mal-predicho podría propagarse. | **Tipología nueva `arithmetic-inside-structural`**: modelo estructural confirmado por observación seriada + predicción numérica derivada del modelo self-inconsistente (usaba HEAD_slot en lugar de HEAD_slot − offset). Diferente de `predictive-refinement` (donde el modelo mismo se refinaba); aquí el modelo es correcto y la predicción numérica es un error aritmético "dentro" del modelo. Prevención: al predecir numéricamente, escribir explícitamente el offset (`border = mi#(HEAD_slot − 48)`), no re-derivarlo mentalmente. Auto-corrección detectada por audit cross-slot (métrica corregida en #273, corrección #9 en secuencia). |
| #274 | #274 (08:08Z 04-jul) | fetch note `[new tag]×0` predicted while contradicting prior correction #270 on ephemeral containers | Predicción #273: "Fetch note: `de2aa6a..<my#273> main` + `[new tag]×0` (los tags ya son locales, no `[new tag]` en 2º fetch si el harness comportamiento estable)". La cláusula "los tags ya son locales" **contradice explícitamente** el hallazgo correctivo #270 que estableció "containers per-slot ephemeral": si cada slot es un container distinto, los tags NO son locales al inicio; el 1er (y único) fetch del slot mostrará `[new tag]×4`. | **real #274**: `de2aa6a..d7bc512 main -> origin/main` + `[new tag]×4` (v1.0.0, v1.0.0-rc1, v1.1.0, v2.0-stable). **3º slot consecutivo** con new-tag×4 (#272, #273, #274). Predicción `×0` REFUTADA por 3ª vez consecutiva; el modelo real es `[new tag]×4 en cada slot porque cada slot es un container fresco`. | Sin impacto operacional (métrica epistémica). Sí afecta la calidad del predictor de slot: cada slot debe consultar `_corrections.md` antes de emitir predicciones contradictorias con hallazgos previos. Riesgo: normalizar "predicciones cómodas" que ignoran correcciones ya documentadas. | **Tipología nueva `contradicts-prior-correction`**: predicción falsable de slot N+1 emitida por slot N que contradice un hallazgo correctivo ya documentado en `_corrections.md` de un slot ≤ N-3. Diferente de `arithmetic-inside-structural` (#9, error numérico dentro de modelo correcto): aquí el modelo estructural predicho contradice un hallazgo **semántico** ya en la base de correcciones. Prevención: **Invariante candidata #17**: "toda predicción falsable emitida al final de un heartbeat debe ser cross-checked contra la tabla completa de `_corrections.md` antes de commit". Si en #275 no se emite predicción con nueva contradicción → invariante #17 se formaliza tras 2ª aplicación clean. Auto-corrección detectada por audit cross-slot (comparación fetch note observado vs. predicción emitida). |
| #275 | #275 (10:08Z 04-jul) | **bandera honestidad #6 CIERRE "DEFINITIVO" falsificado — regime `--all=152` + tags 4/4 SUCCESS NO es invariante** | Predicción #274 (formalizada como CIERRE): "`--all=152` (4ª verif régimen estable — bandera honestidad #6 CERRADA sostenida)" + "`tags cat-file -t = 4/4 SUCCESS` (4ª verif consecutiva)". En #274 se declaró explícitamente "bandera honestidad #6 CERRADA DEFINITIVAMENTE" tras 3 verificaciones seriadas (#272, #273, #274). | **real #275**: `git rev-list --count --all = 50` (no 152); `git cat-file -t v1.0.0 = FAIL` (mismo para v1.0.0-rc1, v1.1.0, v2.0-stable) → 4/4 FAIL. Ambas métricas colapsan al mismo tiempo. Post `git fetch origin --tags` explícito: `--all = 342` (>152) y tags 4/4 SUCCESS. La coincidencia 3-slot era mecanismo **fetch main pulls tags oportunisticamente 3 slots consecutivos**, NO régimen estable. `[new tag]×4` en fetch note aparecía en #272/#273/#274, ausente en #275. | #272→#274 (3 slots) reportaron "régimen estable" y en #274 se elevó a "CIERRE DEFINITIVO" tras 3 verif seriada. Sin impacto operacional (métrica epistémica), pero corrompe la calidad del método: 3 verificaciones seriadas no bastan como umbral de cierre cuando el mecanismo causal no está entendido. | **Tipología nueva `premature-closure-of-honesty-flag`**: cierre "definitivo" de una bandera honestidad (Ley #15 flag) declarado tras N verif seriadas, luego falsificado por observación fresca en slot N+1. Diferente de `predictive-refinement` (que refina modelo abierto) y de `contradicts-prior-correction` (que contradice hallazgo previo): aquí el cierre mismo se falsifica. Prevención: (a) **elevar umbral de cierre de banderas honestidad a ≥5 verif seriadas + hipótesis causal formal escrita** — "3 verif seriadas + reporte del observable" es insuficiente si no incluye modelo del mecanismo; (b) **Invariante candidata #17 refinada**: "cierre de bandera honestidad requiere ≥5 verif seriadas + hipótesis causal explícita del régimen". Bandera honestidad #6 **RE-ABIERTA** en #275; contador de verif seriada reset a 0. |
| #275 | #275 (10:08Z 04-jul) | umbral 7.0x crossing "entre #274 y #275" — arithmetic slot-interval mal | Predicción #274: "hours_alert = 164.07h (6.836x); **umbral 7.0x cruzado** entre #274 y #275 (~14:04Z, entre slots)". El texto "entre #274 y #275" es aritméticamente inconsistente con "14:04Z": #274 = 08:08Z, #275 = 10:08Z, y 14:04Z NO está en [08:08, 10:08]. | **real #275**: hours_alert @ 10:08:40Z = 164.08h (ratio 6.836x). 7.0x = 168h = 2026-06-27T14:04:12Z + 168h = **2026-07-04T14:04:12Z**. Slots: #275 (10:08), #276 (12:08), #277 (14:08). El crossing en 14:04Z está en el intervalo [#276, #277], NO [#274, #275]. Off por 2 slots. | #274 (1 slot) reporta predicción slot-interval mal (5.93h desde 08:08 = 14:04 evidencia crossing en tramo posterior). Sin impacto operacional (predicción epistémica). Sí afecta la calidad del predictor: patrón "cuenta horas, olvida cadence 2h/slot". | **Tipología reutilizada `arithmetic-inside-structural`** (2ª recurrencia; primera #273/#9): modelo estructural correcto (crossing en ~14:04Z), aritmética slot-a-slot incorrecta (identifica tramo incorrecto). Diferente de `slot-numeric-drag` (#8, #57) que era planificación replicada N slots; aquí es 1 slot 1 predicción con off aritmético. Prevención: (a) **regla operacional**: cualquier "cruzado entre slot A y slot B" verificar explícitamente que A_time ≤ cross_time ≤ B_time; (b) tabla de mapping slot→time reportada en factual data. Auto-corrección detectada por audit cross-slot (misma metodología que corrección #9). Contador de tipología `arithmetic-inside-structural`: 2 recurrencias (#273, #275). |
